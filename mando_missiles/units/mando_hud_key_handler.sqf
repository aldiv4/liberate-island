...
private ["_unit", "_list", "_pos", "_ttypes", "_delta", "_log", "_target", "_airtargets", "_structtargets","_mis", "_can_fire", "_fire", "_missparams", "_msg", "_log_target", "_dir", "_launcher", "_old_mode", "_t0", "_ecm", "_activer", "_firer1", "_firer2", "_time_to_fire", "_widx", "_continue", "_ammo"];
...
//      hint format["%1", mando_missileidxbase];
   ...
   if (mando_hud_missile >= 0) then
   {
      ...
      if (!isNull (uiNameSpace getVariable "mandotarget_disp")) then
      {
         ...
         _speedini = (speed vehicle player)/3.6 + ((mando_hud_misp select mando_hud_missile) select 5);
