...
_coords set [2, [0.20, 0.28]]; // IDC 108 Weapon
_coords set [3, [-2.0, -2.0]]; // IDC 109 Ammo
_coords set [4, [-2.0, -2.0]]; // IDC 111 Flares
_coords set [5, [0.20, 0.64]]; // IDC 113 Range
...
_hud_show = {
	...
	(_display DisplayCtrl 51) CtrlSetPosition [0.500 - 0.175, 0.750 - 0.028, 0.350, 0.056];
	(_display DisplayCtrl 51) CtrlCommit 0;
	if (_this) then {
/*
		(_display DisplayCtrl 52) CtrlSetPosition [0.500 - (0.025 * 3 / 4) + 0.000 * 0.150 / 180, 0.750 - 0.025, 0.050 * 3 / 4, 0.050];
		(_display DisplayCtrl 52) CtrlCommit 0.00;
*/
...
_hud_mono = {
	for [{_i = 0}, {_i < count mando_hud_colors}, {_i = _i + 1}] do {
		mando_hud_colors set [_i, [1.0, 1.0, 1.0]];
	};
};
...
[_vehicle] Spawn {
	private["_vehicle", "_to_sleep", "_d_x", "_d_y", "_was", "_dif", "_display", "_had_null_display", "_angroll", "_angpitch", "_vspeed", "_poss", "_posc", "_possa", "_anga"];
	DisableSerialization;
...
Sleep 1.0; (_display DisplayCtrl 76) CtrlSetStructuredText ParseText Format ["<t font='Bitstream' size='1.75'>MMA v%1<br />Initialising...OK<br />Port...OK<br />Starboard...OK<br />Stores...</t>", mando_missile_version];
Sleep 1.0; (_display DisplayCtrl 76) CtrlSetStructuredText ParseText Format ["<t font='Bitstream' size='1.75'>MMA v%1<br />Initialising...OK<br />Port...OK<br />Starboard...OK<br />Stores...OK<br />Loading.</t>", mando_missile_version];
Sleep 1.0; (_display DisplayCtrl 76) CtrlSetStructuredText ParseText Format ["<t font='Bitstream' size='1.75'>MMA v%1<br />Initialising...OK<br />Port...OK<br />Starboard...OK<br />Stores...OK<br />Loading..</t>", mando_missile_version];
Sleep 1.0; (_display DisplayCtrl 76) CtrlSetStructuredText ParseText Format ["<t font='Bitstream' size='1.75'>MMA v%1<br />Initialising...OK<br />Port...OK<br />Starboard...OK<br />Stores...OK<br />Loading...</t>", mando_missile_version];
	...
	while {!mando_exit_hud} do 
	{
		...
		if ((count _pos_eye1 == 3) && (count _pos_eye2 == 3)) then 
		{
			...
			if (!IsNull _display) then 
			{
				...
				{
					_had_null_display = false;
					(_display DisplayCtrl 52) CtrlSetPosition [0.500 - (0.025 * 3 / 4) + _delta * 0.150 / 180, 0.750 - 0.025, 0.050 * 3 / 4, 0.050];
