...
//               _tti = _tti + 0.5;
//               _tti = _tti / _rounds_salvo;
               for [{_j = 0},{_j < _rounds_salvo},{_j = _j + 1}] do
               {
                  _tti = (_disp distance _blanco)/_muzzlevel;
                  _posfoe = getPosASL _blanco;
                  _vel = velocity _blanco;
                  _posfoe = [(_posfoe select 0)+(_vel select 0)*_tti+(_j/_rounds_salvo)*4,(_posfoe select 1)+(_vel select 1)*_tti+(_j/_rounds_salvo)*4,(_posfoe select 2)+(_vel select 2)*_tti+(_j/_rounds_salvo)*4];
