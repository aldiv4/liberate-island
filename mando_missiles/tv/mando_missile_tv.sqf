/*********************************************************************************
   mando_missile_tv.sqf v1.3
   by Mandoble, August 2009
...
// Back color has no effect, use only pic if exist
      ...
      else
      {
         ...
         {
            (_display displayCtrl 23918) ctrlSetText format["Fire: %1", mando_tv_plane ammo mando_tv_weaponname];
            if ((isNull (gunner mando_tv_plane)) && 
                (count (configFile>>"cfgVehicles" >> (typeOf mando_tv_plane) >> "Turrets") > 0)) then
            {
               player action ["MANUALFIRE", vehicle player];
            ...
            while {!isNull _commdisplay} do
            {
               if (mando_tv_cam == 1) then
               {
                  if (!isNull mando_last_missile) then
                  {
                     ...
                     {
                        _cam attachTo [mando_last_missile, [0,7,0]]; 
                           ...
                           _list = [];
                           {
                              if (fuel _x < 1) then
                              {
                                 if (isEngineOn _x) then
                                 {
                                    ...
                                    {
                                       if ([_loglos, mando_last_missile, _x, 20, _deltaz] call mando_check_los) then
                                       {
                                          _list = _list + [_x];
                                       };
                                    };
...
// CAM MOD                              _cam camSetTarget _mando_tv_cam_target;
                           ...
                           _textdata ctrlSetStructuredText parseText format["Warning: %1<br />Zoom: %2<br />Range to destination: %3 Km%4", _warning, floor((1 - mando_tvcam_scale)*100/0.99), floor ((mando_tv_plane distance mando_tv_dest)/100)/10, _range_to_target];
                                          ...
                                          if (isEngineOn _x) then
                                          {
                                             if (side _x != side mando_tv_plane) then 
                                             {
                                                if (((getPos _x select 2) > _min_targets_alt) &&
                                                    ((getPos _x select 2) < _max_targets_alt)) then
                                                {
                                                   if ([_loglos, mando_tv_plane, _x, 20, _deltaz] call mando_check_los) then
                                                   {
                                                      _list = _list + [_x];
                                                   };
                                                };
...
*/
   ...
   case "ZOOM_IN":
   {
      ...
      if (mando_tvcam_scale > 0.1) then
      {
         mando_tvcam_scale = mando_tvcam_scale - 0.1;
      };
   };
      ...
      if (alive mando_tv_plane && !(isNil "mando_missile_init")) then
      {
         ...
         if (!isNull _target) then
         {
            ...
            else
            {
               if (mando_tv_weaponname != "") then
               {
                  if ((mando_tv_plane ammo mando_tv_weaponname) > 0) then
                  {
                     ((findDisplay 23900) displayCtrl 23918) ctrlSetText format["Fire: %1", (mando_tv_plane ammo mando_tv_weaponname)-1];
                     if ((mando_tv_plane ammo mando_tv_weaponname) == 1) then
                     {
                        playSound "mando_noammo1";
                        playSound "mando_noammo2";
                     };
            ...
            if (_continue) then
            {
               ...
               if (!mando_tv_man) then
               {
                  ...
                  // BIS missile
                     ...
                     mando_firing = true;
                     _ammo = mando_tv_plane ammo mando_tv_weaponname;
                     ...
                     {
                        if ((mando_tv_plane ammo mando_tv_weaponname) < _ammo) then
                        {
                           _missparam set [2, [((_missparam select 2) select 0)*mando_tv_pilon, ((_missparam select 2) select 1),((_missparam select 2) select 2)]];
