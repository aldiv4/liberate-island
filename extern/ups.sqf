...
// ***************************************** SERVER INITIALIZATION *****************************************
	...
	// global functions
if (isNil("WC_KRON_UPS_INIT")) then {
	WC_KRON_UPS_INIT=0;

	// find a random position within a radius
	WC_KRON_randomPos = {private["_cx","_cy","_rx","_ry","_cd","_sd","_ad","_tx","_ty","_xout","_yout"];_cx=_this select 0; _cy=_this select 1; _rx=_this select 2; _ry=_this select 3; _cd=_this select 4; _sd=_this select 5; _ad=_this select 6; _tx=random (_rx*2)-_rx; _ty=random (_ry*2)-_ry; _xout=if (_ad!=0) then {_cx+ (_cd*_tx - _sd*_ty)} else {_cx+_tx}; _yout=if (_ad!=0) then {_cy+ (_sd*_tx + _cd*_ty)} else {_cy+_ty}; [_xout,_yout]};
	// find any building (and its possible building positions) near a position
	WC_KRON_PosInfo = {private["_pos","_lst","_bld","_bldpos"];_pos=_this select 0; _lst= nearestObjects [_pos,["House","vbs2_house"],20]; if (count _lst==0) then {_bld=0;_bldpos=0} else {_bld=_lst select 0; _bldpos=[_bld] call WC_KRON_BldPos}; [_bld,_bldpos]};
	/// find the highest building position	
	WC_KRON_BldPos = {private ["_bld","_bi","_bldpos","_maxZ","_bp","_bz","_higher"];_bld=_this select 0;_maxZ=0;_bi=0;_bldpos=0;while {_bi>=0} do {_bp = _bld BuildingPos _bi;if ((_bp select 0)==0) then {_bi=-99} else {_bz=_bp select 2; _higher = ((_bz>_maxZ) || ((abs(_bz-_maxZ)<.5) && (random 1>.5))); if ((_bz>4) && _higher) then {_maxZ=_bz; _bldpos=_bi}};_bi=_bi+1};_bldpos};
	WC_KRON_OnRoad = {private["_pos","_car","_tries","_lst"];_pos=_this select 0; _car=_this select 1; _tries=_this select 2; _lst=_pos nearRoads 4; if ((count _lst!=0) && (_car || !(surfaceIsWater _pos))) then {_tries=99}; (_tries+1)};
	WC_KRON_getDirPos = {private["_a","_b","_from","_to","_return"]; _from = _this select 0; _to = _this select 1; _return = 0; _a = ((_to select 0) - (_from select 0)); _b = ((_to select 1) - (_from select 1)); if (_a != 0 || _b != 0) then {_return = _a atan2 _b}; if ( _return < 0 ) then { _return = _return + 360 }; _return};
	WC_KRON_distancePosSqr = {(((_this select 0) select 0)-((_this select 1) select 0))^2 + (((_this select 0) select 1)-((_this select 1) select 1))^2};
	WC_KRON_relPos = {private["_p","_d","_a","_x","_y","_xout","_yout"];_p=_this select 0; _x=_p select 0; _y=_p select 1; _d=_this select 1; _a=_this select 2; _xout=_x + sin(_a)*_d; _yout=_y + cos(_a)*_d;[_xout,_yout,0]};
	WC_KRON_rotpoint = {private["_cp","_a","_tx","_ty","_cd","_sd","_cx","_cy","_xout","_yout"];_cp=_this select 0; _cx=_cp select 0; _cy=_cp select 1; _a=_this select 1; _cd=cos(_a*-1); _sd=sin(_a*-1); _tx=_this select 2; _ty=_this select 3; _xout=if (_a!=0) then {_cx+ (_cd*_tx - _sd*_ty)} else {_cx+_tx}; _yout=if (_a!=0) then {_cy+ (_sd*_tx + _cd*_ty)} else {_cy+_ty}; [_xout,_yout,0]};
	WC_KRON_stayInside = {
		...
		_tp = [_cp,_d,(_nx-_cx),(_ny-_cy)] call WC_KRON_rotpoint;
		...
		if (_ty>(_cy+_ry)) then {_fy=_cy+_ry};
		if ((_fx!=_tx) || (_fy!=_ty)) then {_np = [_cp,_d*-1,(_fx-_cx),(_fy-_cy)] call WC_KRON_rotpoint};
		_np;
	};
	// Misc
	...
	WC_KRON_AllEast=[];
	WC_KRON_AllRes=[];
	WC_KRON_KnownEnemy=[objNull,objNull];
	...
	// find all units in mission
	{
		...
		switch (_s) do {
			case west: 
				{ WC_KRON_AllWest=WC_KRON_AllWest+[_x]; };
			case east: 
				{ WC_KRON_AllEast=WC_KRON_AllEast+[_x]; };
			case resistance: 
	...
	WC_KRON_HQ="Logic" createVehicle [0,0];
	WC_KRON_UPS_Instances=0;
	WC_KRON_UPS_Total=0;
	WC_KRON_UPS_Exited=0;
	WC_KRON_UPS_INIT=1;
};
...
_grpidx = format["%1",WC_KRON_UPS_Instances];
...
_members = units _npc;
WC_KRON_UPS_Total = WC_KRON_UPS_Total + (count _members);
...
if (_issoldier) then {
	switch (side _npc) do {
		case west:
			{ _friends=_friends+WC_KRON_AllWest; _enemies=_enemies+WC_KRON_AllEast+WC_KRON_AllRes; _sharedenemy=0; };
		case east:
			{ _friends=_friends+WC_KRON_AllEast; _enemies=_enemies+WC_KRON_AllWest+WC_KRON_AllRes; _sharedenemy=1; };
		case resistance:
			{ _enemies=_enemies+WC_KRON_AllEast+WC_KRON_AllWest; _sharedenemy=2; };
	};
	{
...
call compile format ["WC_KRON_UPS_%1=1",_npcname];
...
_orgPos = _currPos;
_orgWatch=[_currPos,50,getDir _npc] call WC_KRON_relPos; 
_orgDir = getDir _npc;
_avoidPos = [0,0];
_flankPos = [0,0];
_attackPos = [0,0];
...
// adjust cycle delay 
_cycle = ["CYCLE:",5,_UCthis] call WC_KRON_getArg;
...
if (_initpos!="ORIGINAL") then {
	...
	_bldpos=0;
	while {_try<20} do {
		_currPos=[_centerX,_centerY,_rangeX,_rangeY,_cosdir,_sindir,_areadir] call WC_KRON_randomPos;
		if ((_initpos=="RANDOMUP") || ((_initpos=="RANDOM") && (random 1>.75))) then {
			_posinfo=[_currPos] call WC_KRON_PosInfo;
...
_mincopies = ["MIN:",0,_UCthis] call WC_KRON_getArg;
_maxcopies = ["MAX:",0,_UCthis] call WC_KRON_getArg;
...
if (_maxcopies>0) then {
	...
	// any init strings?
	_initstr = ["INIT:","",_UCthis] call WC_KRON_getArg;

// name of clones
_nameprefix = ["PREFIX:","UPSCLONE",_UCthis] call WC_KRON_getArg;
...
// create the clones
	for "_grpcnt" from 1 to _copies do {
		...
		// copy groups
		if (isNil ("WC_KRON_cloneindex")) then {
			WC_KRON_cloneindex = 0;
		}; 
		...
		WC_KRON_cloneindex = WC_KRON_cloneindex+1;
		_lead setVehicleVarName format["%1%2",_nameprefix,WC_KRON_cloneindex];
		call compile format["%1%2=_lead",_nameprefix,WC_KRON_cloneindex];
		...
		_c=0;
		{
			...
			if (_c>1) then {
				...
				WC_KRON_cloneindex = WC_KRON_cloneindex+1;
				_newunit setVehicleVarName format["%1%2",_nameprefix,WC_KRON_cloneindex];
				call compile format["%1%2=_newunit",_nameprefix,WC_KRON_cloneindex];
...
_zoneempty = ["EMPTY:",0,_UCthis] call WC_KRON_getArg;
...
if (_usetrigger!="NOTRIGGER") then {
	...
	call compile format["WC_KRON_TRGFlag=%1",_trgname];
	if (isNil ("WC_KRON_TRGFlag")) then {
...
if (_exit) exitWith {
	if ((WC_KRON_UPS_DEBUG>0) && !_onroof) then {hint "Initialization aborted"};
};
...
while {_loop} do {
	...
	if ((_issoldier) && ((count _enemies)>0) && !(_exit)) then {
		...
		if ((_shareinfo=="SHARE") && (behaviour _npc=="SAFE")) then {
			...
			{
				if ((!(isNull _x) && (_npc distance _x<WCUPSSHAREDIST)) && ((damage _x>.5) || (behaviour _x in ["AWARE","COMBAT"]))) exitWith {
		...
		_maxknowledge=0;
		{
			...
			if ((alive _x) && (_knows>0.2) && (_knows>_maxknowledge)) then {
				WC_KRON_KnownEnemy set [_sharedenemy,_x]; 
		...
		if (isNull (WC_KRON_KnownEnemy select _sharedenemy)) then {
			_pursue=false;
		};
		...
		if (_pursue) then	{
			...
			_dir1 = [_currPos,_targetPos] call WC_KRON_getDirPos;
			// angle from target to unit (reverse direction)
			_dir2 = (_dir1+180) mod 360;
			// angle from fatality to target
			_dir3 = if (_hitPos select 0!=0) then {[_hitPos,_targetPos] call WC_KRON_getDirPos} else {_dir1};
			_dd=(_dir1-_dir3);
			
			// unit position offset straight towards target
			_relUX = sin(_dir1)*WCUPSSAFEDIST; _relUY = cos(_dir1)*WCUPSSAFEDIST;
			// target position offset straight towards unit
			_relTX = sin(_dir2)*WCUPSSAFEDIST; _relTY = cos(_dir2)*WCUPSSAFEDIST;
			...
			if (_nofollow=="NOFOLLOW") then {
				_avoidPos = [_avoidPos,_centerpos,_rangeX,_rangeY,_areadir] call WC_KRON_stayInside;
				_flankPos = [_flankPos,_centerpos,_rangeX,_rangeY,_areadir] call WC_KRON_stayInside;
				_attackPos = [_attackPos,_centerpos,_rangeX,_rangeY,_areadir] call WC_KRON_stayInside;
				_targetPos = [_targetPos,_centerpos,_rangeX,_rangeY,_areadir] call WC_KRON_stayInside;
			};
	...
	if !(_newpos) then {
		...
		if (_waiting<=0) then {
			// distance to target
			_dist = [_currPos,_targetPos] call WC_KRON_distancePosSqr;
			if (_lastdist==0) then {_lastdist=_dist};
			_moved = abs(_dist-_lastdist);
			// adjust the target tolerance for fast moving vehicles
			if (_moved>_maxmove) then {_maxmove=_moved; if ((_maxmove/40) > _WCUPSCLOSEENOUGH) then {_WCUPSCLOSEENOUGH=_maxmove/40}};
			...
			if ((!_swimming) && ((_dist<=_WCUPSCLOSEENOUGH) || (_totmove<.2) || (_dammchg>0.01) || (_timeontarget>WCUPSALERTTIME))) then {_makenewtarget=true;};

			// in 'attack (approach) mode', so follow the flanking path (don't make it too predictable though)
			if ((_fightmode!="walk") && (_dist<=_WCUPSCLOSEENOUGH)) then {
				if ((random 1)<.95) then {
			...
			if (_makenewtarget) then {
				if ((_nomove=="NOMOVE") && (_timeontarget>WCUPSALERTTIME)) then {
					if (([_currPos,_orgPos] call WC_KRON_distancePosSqr)<_WCUPSCLOSEENOUGH) then {
						_newpos = false;
					} else {
						_targetPos=_orgPos;
					};
					...
					_targetPos=_currPos;
					_tries=0;
					while {((([_currPos,_targetPos] call WC_KRON_distancePosSqr) < _mindist)) && (_tries<20)} do {
						_tries=_tries+1;
						// generate new target position (on the road)
						_tries=0;
						while {_tries<20} do {
							_targetPos=[_centerX,_centerY,_rangeX,_rangeY,_cosdir,_sindir,_areadir] call WC_KRON_randomPos; 
							if (_iscar) then {
	...
	if (surfaceIsWater _currPos) then {
		if (!_swimming) then {
			...
			for [{_a=0}, {_a<=270}, {_a=_a+90}] do {
				_dp=[_currPos,30,_a] call WC_KRON_relPos; 
	...
	if ((_waiting<=0) && _newpos) then {
		...
		if (_fightmode!="walk") then {
			...
			if (_timeontarget>WCUPSALERTTIME) then {
				_fightmode="walk";
				_speedmode = _orgSpeed;
				{
	...
	_cont = (call compile format ["WC_KRON_UPS_%1",_npcname]);
