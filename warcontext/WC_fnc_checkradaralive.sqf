	...
	private ["_radar", "_side", "_array", "_vehicle", "_pilot", "_position", "_size"];
	...
	WC_fnc_check_radarenemy = {
		...
		_size = [(getposasl _radar) select 2] call WC_fnc_checkcoverage;
	...
	if(_side == wcside) then {
		_marker = ["WRADARMRK", [(getposasl _radar) select 2] call WC_fnc_checkcoverage, position _radar, 'ColorBLUE', 'ICON', 'FDIAGONAL', 'EMPTY', 0, '', true] call WC_fnc_createmarker;
		...
		wcgarbage = [_radar, _side, _vehicle] spawn WC_fnc_check_radarenemy;
	} else {

		_marker = ["ERADARMRK", [(getposasl _radar) select 2] call WC_fnc_checkcoverage, position _radar, 'ColorBLUE', 'ICON', 'FDIAGONAL', 'EMPTY', 0, '', true] call WC_fnc_createmarker;
		_position = [_marker, "onmountain"] call WC_fnc_createpositioninmarker;
